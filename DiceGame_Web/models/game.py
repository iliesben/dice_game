    ...
    def launch_game(self,player_list):
        ...
        while self.max_score <= params.DEFAULT_TARGET_SCORE:
            ...
            for player in list_player:
                ...
                best_player_score = app_stat.best_player_score(player)
                best_longest_turn = app_stat.best_longest_turn(app_stat.longest_turn)
                best_turn_loss = app_stat.best_turn_loss(player)
                if nbr_scoring_dice == 0 :
                    mean_no_scoring_turn = app_stat.mean_non_scoring_turn(player)
                if nbr_scoring_dice != 0:
                    mean_scoring_turn = app_stat.mean_scoring_turn(player)

                if player.score > params.DEFAULT_TARGET_SCORE:
                    list_player.sort(key=attrgetter('score'), reverse=True)
                    return self.display_party(), print(best_player_score), print(best_longest_turn), print(best_turn_loss), print(mean_no_scoring_turn), print(mean_scoring_turn)
            self.turn += 1
